...
// + вычисляемые каналы OUT2..4 (64-66; OUT1 = курируемая серия 'co1').
...
const isAfrTarget = (l: string) =>
	// Дефолтная короткая подпись MS3 "AFT" (afrtgt1 = Bank 1 AFR target) — в ней нет "afr".
	// \b — чтобы не ловить "shaft"/"draft"/"after".
	/\baft\b/.test(l) ||
